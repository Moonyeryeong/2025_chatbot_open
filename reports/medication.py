...
if med_list:
    ...
    if not date_df.empty:
        ...
        for i, (_, row) in enumerate(date_df.iterrows()):
            col1, col2, col3, col4, col5 = st.columns([2,2,2,2,1])
            col1.write(row["약 이름"])
            col2.write(row["date_str"])
            col3.write(row["복용 시간"])
            col4.write(row["비고"])
            # i를 키에 포함시키면 중복 키 오류가 사라집니다
            btn_key = f"del_{i}_{row['약 이름']}_{row['date_str']}_{row['복용 시간']}"
            if col5.button("삭제", key=btn_key):
                # 삭제 로직
                for j, m in enumerate(med_list):
                    if (m["약 이름"] == row["약 이름"]
                        and m["복용 날짜"] == row["date_str"]
                        and m["복용 시간"] == row["복용 시간"]):
                        med_list.pop(j)
                        break
                save_medications(user_id, med_list)
                st.success("삭제되었습니다.")
